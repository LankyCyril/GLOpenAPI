...
from glopenapi.db.sql.utils import validate_no_backtick, validate_no_doublequote
...
from glopenapi.common.utils import as_is
from glopenapi.common.exceptions import GLOpenAPIDatabaseException
from math import inf
from collections import OrderedDict
from glopenapi.db.sql.streamed_tables import StreamedDataTableWizard_Single
from os import path
...
class SQLiteObject():
    ...
    def is_stale(self, *, timestamp_table=None, id_field=None, db_type=None, ignore_conflicts=False):
        ...
        if (timestamp_table is None) or (id_field is None):
            ...
            GLOpenAPILogger.warning(f"{type(self).__name__} {msg}")
        else:
        ...
        if (_staleness is None) and (not ignore_conflicts):
            with self.sqltransactions.exclusive(desc) as (connection, _):
                ...
                GLOpenAPILogger.warning(f"{msg}\n  ({self_id_value})")
                self.drop(connection=connection)
            _staleness = True
        if _staleness is True:
            GLOpenAPILogger.info(f"{self_id_value} is stale, staging update")
        return _staleness
...
class SQLiteBlob(SQLiteObject):
    ...
    def __init__(self, *, sqlite_db, identifier, table, timestamp, compressor=None, decompressor=None, maxdbsize=None):
        if not table.startswith("BLOBS:"):
            ...
            raise GLOpenAPIConfigurationException(msg, table=table)
...
class SQLiteTable(SQLiteObject):
    ...
    def __init__(self, *, sqlite_db, table, aux_table, timestamp, maxpartcols=998, maxdbsize=None):
        if not table.startswith("TABLE:"):
            ...
            raise GLOpenAPIConfigurationException(msg, aux_table=aux_table)
        else:
    ...
    def retrieve(self, desc="tables/retrieve"):
        ...
        if not column_dispatcher:
            raise GLOpenAPIDatabaseException("No data found", table=self.table)
        else:
    ...
    def cleanup(self, max_iter=100, max_skids=20, desc="tables/cleanup"):
        ...
        desc = f"SQLiteTable():\n  {self.sqlite_db}"
        if n_dropped:
            GLOpenAPILogger.info(f"{desc} shrunk by {n_dropped} entries")
        elif path.getsize(self.sqlite_db) > self.maxdbsize:
            GLOpenAPILogger.warning(f"{desc} could not be shrunk")
        if n_skids:
            GLOpenAPILogger.warning(f"{desc} did not shrink {n_skids} times")
