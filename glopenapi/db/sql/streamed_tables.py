...
from glopenapi.db.sql.utils import validate_no_backtick
...
from glopenapi.common.hacks import apply_hack, speed_up_data_schema
    ...
    def __init__(self, *, sqlite_db, query, targets, kind="TABLE", _depends_on=None, msg=None):
        ...
        with self.sqltransactions.exclusive("TempSelect") as (_, execute):
            ...
            try:
                ...
                GLOpenAPILogger.info(f"{msg} {self.name} from\n  {query_repr}")
...
class StreamedDataTableWizard():
    ...
    def columns(self, passed_columns):
        ...
        if passed_last_level <= own_last_level:
            ...
            raise GLOpenAPIFileException(msg, columns=foreign)
        ...
        if isinstance(full_name, Iterable): # string or tuple or list
            ...
            if full_name == 0:
                if passed_name == self._index_name:
                    ...
                    raise GLOpenAPIFileException(msg, column=passed_name)
            else:
    ...
    def _sanitize_where(self, context):
        ...
        for dc in getattr(context, "data_comparisons", []):
            ...
            if not match:
                ...
                raise GLOpenAPIFileException(msg, comparison=dc)
            else:
    ...
    def _make_query_filter(self, context, limit, offset):
        ...
        if (offset != 0) and (limit is None):
            ...
            raise GLOpenAPIDatabaseException(msg, table=self.name)
    ...
    def get(self, *, context, limit=None, offset=0):
        ...
        GLOpenAPILogger.info(f"{self.name};\n  {msg}")
        return data
