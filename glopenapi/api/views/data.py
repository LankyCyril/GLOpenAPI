...
from glopenapi.db.mongo.utils import match_sample_names_to_file_descriptor
...
def harmonize_columns(columns, descriptor, sample_names, best_sample_name_matches):
    ...
    for i, c in enumerate(columns):
        ...
        if len(hcs) == 0:
            ...
            raise GLOpenAPIDataManagerException(msg, **_kws)
...
def combine_objects(objects, context, limit=None):
    ...
    if isinstance(combined, StreamedDataTableWizard):
        if context.data_columns and (context.format == "gct"):
            ...
            raise GLOpenAPIFormatException(msg, **_kw)
        else:
...
def combined_data(descriptors, n_descriptors, context, mongo_collections, sqlite_dbs, adapter):
    ...
    if getset("file", "cacheable") != {True}:
        ...
        sug = "Use 'format=raw'"
        raise GLOpenAPIFileException(msg, suggestion=sug, format=context.format)
    ...
    if data is None:
        raise GLOpenAPIDatabaseException("No data found in database")
    else:
...
def get(*, mongo_collections, locale, context, sqlite_dbs, adapter):
    ...
    for n_descriptors, d in enumerate(descriptors, 1):
        if ("file" in d) and (not isinstance(d["file"], dict)):
            ...
            raise GLOpenAPIDatabaseException(msg, debug_info=d)
        elif ("file" not in d) or ("filename" not in d["file"]):
            msg = "File information missing for entry"
            raise GLOpenAPIDatabaseException(msg, entry=d)
    if n_descriptors == 0:
